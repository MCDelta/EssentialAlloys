...
public class BlockCrusher extends BlockSided implements ITileEntityProvider
{
     ...
     public void registerIcons (IconRegister register)
     {
          ...
          this.sideIcon = doRegister(this.name + "_side_off", register);
          this.sideIconOn = doRegister(this.name + "_side_on", register);
          this.insideIcon = doRegister(this.name + "_inside", register);
     ...
     public void setBlockBoundsBasedOnState (IBlockAccess world, int x, int y, int z)
     {
          ...
          if (((TileEntityCrusher) pos.getTile()) != null && (((BlockCrusher) BlockEA.crusher).isExtended(pos) || ((TileEntityCrusher) pos.getTile()).extend != 0))
          {
     ...
     public void doThings (Position pos)
     {
          ...
          if (!isExtended(pos) && Assets.isPoweredIndirectly(pos))
          {
               ...
               if (extend(pos))
               {
                    ...
                    {
                         PacketDispatcher.sendPacketToAllAround(pos.x, pos.y, pos.z, 20, ((World) pos.blockAccess).provider.dimensionId, Assets.populatePacket(new PacketCrusherExtend(tile.extend, pos.x, pos.y, pos.z)));
                    }
